...
def create_student():                                                                                       # Creating CREATE CRUD Operator
    name = entry_name.get()
    ...
    db.commit()
    messagebox.showinfo("Success", "Student record created successfully.")                                  # Popup Message after UPLOAD on MySQL

def reset_values():                                                                                         # Resetting Function to NULL after Uploaded 
...
window = tk.Tk()                                                                                            # Window Tkinter
window.title("Student Registration")                                                                        # Title for Window
window.configure(bg="#303030")                                                                              # Changing Background Color for Window


label_name = tk.Label(window, text="Name :")                                                                # Creating Label to Enter Data
...
departments = ["CSE", "MECH", "ECE", "EEE", "AI & DS", "AUTO", "CIVIL", "EIE", "IT"]                      # Array to hold Dropdown Menu Values

selected_department = tk.StringVar(window)                                                                # DropDown Menu for Department
selected_department.set("Select")
...
button_create = tk.Button(window, text="UPLOAD", command=create_student, bg="#0077b6", fg="#ffffff")     # Creating Button to UPLOAD into MySQL
button_create.grid(row=8, column=0, padx=10, pady=5)

button_reset = tk.Button(window, text="RESET", command=reset_values, bg="#0077b6", fg="#ffffff")         # Creating Button to Reset residue values 
